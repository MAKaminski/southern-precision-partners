...
const STATUSES = [
  { value: "tbd", label: "TBD", className: "bg-surface text-text-secondary border-border-custom" },
  { value: "planned", label: "Planned", className: "bg-accent-blue/10 text-accent-blue border-accent-blue/20" },
  { value: "in_progress", label: "In Progress", className: "bg-accent-amber/10 text-accent-amber border-accent-amber/20" },
  { value: "done", label: "Done", className: "bg-accent-green/10 text-accent-green border-accent-green/20" },
] as const;

type Status = (typeof STATUSES)[number]["value"];

export function GrowthInitiativeStatusSelect({ id, initial }: { id: string; initial: Status }) {
  ...
  async function update(next: Status) {
    ...
    setError(false);
    try {
      const res = await fetch(`/api/growth-initiatives/${id}`, {
        method: "PATCH",
